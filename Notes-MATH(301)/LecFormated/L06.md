...
> 
> ($\Leftarrow$) If $x_{m+n}\to x$, then for any $\epsilon>0$ there exists $K(\epsilon)$ such that $|x_{m+n}-x|<\epsilon$ for all $n\ge K(\epsilon)$. Set $N(\epsilon):=m+K(\epsilon)$. Then for any $k\ge N(\epsilon)$ we can write $k=m+n$ with $n\ge K(\epsilon)$, hence $|x_k-x|<\epsilon$. Therefore $x_n\to x$.# Extractions -------------
# Intervals and Sequences

*Extracted to: [[L06 - Intervals and Sequences]]*
