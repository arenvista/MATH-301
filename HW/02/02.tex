...
\[
	\begin{cases}
		\text{Case 1: } (a = 0) & a < b \implies a^2 = ab < b^2                                      \\
		\text{Case 2: } (a > 0) & a < b \implies (a^2 < ab) \land (ab < b^2) \implies a^2 < ab < b^2
	\end{cases}
\]

Note the following:
\[
	(a^2 < ab < b^2) \land (a^2 = ab < b^2) \equiv a^2 \leq ab < b^2 \text{ which is what we wanted to show}
\]

Likewise since if $a=0 \implies  a^2 = ab < b^2$ we can not satify the inequality $a^2 <ab < b^2$
...
\[
	\therefore |a/b| = |a|/|b|
\]
\section*{Question 10: Section 2.2-2}
...
\[
	...
	\end{aligned}
\]
Observe there is no way to satisfy the inequality in (\textbf{Case 2} ). Therefore we can conclude possible solutions may only exist in \textbf{Case 1,3}.
Thus are bounded region with solutions to the inequality are as follows:
\[
	(-\frac{5}{2} < x < -3) \land (\frac{3}{2} < x < 2)
\]
\section*{Question 12: Section 2.2-17}
\begin{center}
	Show that if $a,b \in \mathbb{R}$ and $a \neq b$ then $\exists ~\epsilon \text{-neighborhoods } U \text{ of a and } V \text{ of b} \ni U \cap V = \emptyset$
\end{center}
\[
	\begin{aligned}
		U_{\epsilon}(a):
		 & = \{u \in \mathbb{R} : |u-a| < \epsilon \}            \\
		 & = \{u \in \mathbb{R} : -\epsilon < u-a < \epsilon \}  \\
		 & = \{u \in \mathbb{R} : a-\epsilon < u < \epsilon+a \} \\
		 & = (a-\epsilon, a+\epsilon)                            \\
		V_{\epsilon}(b):
		 & = \{v \in \mathbb{R} : |v-b| < \epsilon \}            \\
		 & = \{v \in \mathbb{R} : -\epsilon < v-b < \epsilon \}  \\
		 & = \{v \in \mathbb{R} : b-\epsilon < v < \epsilon+b \} \\
		 & = (b-\epsilon, b+\epsilon)
	\end{aligned}
\]
Observe $U \cap V$:
\[
	\begin{aligned}
		U \cap V := \{ x \in \mathbb{R} : (|x-a| < \epsilon) \land (|x-b| < \epsilon) \} \\
		\implies \forall ~\epsilon < \frac{1}{2}|a-b|, U \cap V = \emptyset
	\end{aligned}
\]
\end{document}
