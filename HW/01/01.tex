...
\[
	\begin{aligned}
		 & S \lor (y \in T \land  y \not\in S) \\
		 & S \lor (y \in T \land  y \in S^c)   \\
		 & y \in S \cup ( T \cap S^c)          \\
		 & y \in (S \cup T) \cap (S \cup S^c)  \\
		 & (S \cup T) \cap U                   \\
		 & S \cup T
	\end{aligned}
\]
\begin{center}
	Thus we can state that $S \cup T \equiv S \cup T-S$. \\
\end{center}
...
\textbf{Goal:} We want to show that $\forall ~y \in S \cup T, \exists ~n \in C \ni h(n) = y$
    ...
    Thus we can validate a condition in "Countable Set Implications" ($\exists $ surjection $\mathbb{N}$ onto $S$). \\
    It means there must be an injection of $S \cup (T-S) \rightarrow \mathbb{N}$. \\
	\begin{center}
...
\begin{proof} (\textbf{Show:} If $\exists$ a $f: T_1 \rightarrow  T_2$, where $T_2$ is finite and $f$ is bijective. Then $T_1$ is finite.) \\
	...
	\[
		f(x) = f(y) \implies x \neq y
	\]
	This is a contradiction, thus we can say: if $\exists$ a $f: T_1 \rightarrow  T_2$, where $T_2$ is finite and $f$ is bijective. Then $T_1$ is finite.
\end{proof}
% END
