...
\begin{proof} \textbf{Question:} Exhibit a bijection between $\mathbb{N}$ and the set of all odd integers greater than 13.
	\subsection*{Setup}
	Let $A$ be a set defined as:
	\[
		S := \{ z | z \in \mathbb{Z}, z > 13 \}
	\]
	Observe the elements of $S$, where $i \in \mathbb{N}$:
	\begin{align*}
		S_1 = 14 \\
		S_2 = 15 \\
		S_3 = 16 \\
	...
	\]
	We can also say this is the same as:
	\[
		f(n) = 13 + n
	\]
	...
	\begin{proof} \textbf{Goal:} Show that $\forall ~s \in S, \exists ~n \in \mathbb{N} \ni f(n) = s$ \\
		...
		\begin{align*}
			...
			n = s - 13
		\end{align*}
		Clearly $s$ has a infima of 14 as defined by the proposition and must be an integer.
		\begin{center} Therefore $s - 13 \in \mathbb{N}$ \end{center}
		Back solving:
		\begin{align*}
			f(s-13) & = (s-13) + 13 \\
			        & = s -13 + 13  \\
			        & = s + 13
		\end{align*}
		Observe $f(n) = s + 13$ which we obtained and is the expected result as the input to the prior solution and no terms escape the defined sets; thus the statments holds
		\begin{center}
			We have show that $\forall ~s \in S, \exists ~n \in \mathbb{N} \ni f(n) = s$. \\
			Therefore $f$ is surjective.
		\end{center}
	\end{proof}
	\subsection*{Injectivity}
	\begin{proof} \textbf{Goal:} Show that given $a,b \in \mathbb{N}$ and $f(a) = f(b) \implies a = b$
		Let $a,b \in \mathbb{N}$ and $f(a) = f(b)$. Observe:
		\begin{align*}
			f(a)        & = f(b)        \\
			a + 13      & = b + 13      \\
			a + 13 - 13 & = b + 13 - 13 \\
